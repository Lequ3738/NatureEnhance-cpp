...
expReal InstancePlaceList(GMReal x, GMReal y, GMReal id)
{
	...
	if (ind >= 100000)
	{
		int placeID = gm::instance_place(x, y, ind);
		if (placeID >= 0)
		{
			list = static_cast<int>(ne_list_create("instance_place_list() Created List."));
			gm::ds_list_add(list, placeID);
		}

		return static_cast<GMReal>(list);
	}
	
	int num = gm::instance_number(ind);
	for (int i = 0; i < num; ++i)
	{
		int inst = gm::instance_find(ind, i);
		if (gm::place_meeting(x, y, inst))
		{
			if (list < 0)
				list = static_cast<int>(ne_list_create("instance_place_list() Created List."));

			gm::ds_list_add(list, inst);
		}
	}
